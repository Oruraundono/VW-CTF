...
def sweep(s, top_rpm, tag):
    if s.dry:
        s.hold_rpm(top_rpm, 0.8, tag)
        s.hold_rpm(0, 0.5, "drop")
        return
    for r in range(0, top_rpm + 1, 400):
        s.hold_rpm(r, 0.06)
    s.hold_rpm(top_rpm, 0.6, tag)
    for r in range(top_rpm, -1, -400):
        s.hold_rpm(r, 0.06)
    s.hold_rpm(0, 0.6, "drop")


def transmit(s, text, debug=False):
    text = "".join(c for c in text.upper() if c.isalpha())
    top = BASE * STEP_RPM
    sweep(s, top, "START sweep")
    for ch in text:
        n = ord(ch) - 64
        hi, lo = n // BASE, n % BASE
        if debug:
